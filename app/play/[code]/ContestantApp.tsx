      ...
      <div className="max-w-3xl mx-auto space-y-4">
        <header className="card flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="text-right">
            ...
            <div className="text-3xl font-mono font-bold text-ocean-800">{myPoints + (myAnswer?.locked ? 0 : 0)}đ</div>
          </div>
        </header>

        <div className="card space-y-3">
          {phase === "idle" && <Banner>Đang chờ Ban Tổ chức bắt đầu phần thi...</Banner>}
          {phase === "armed" && currentQuestion && <Banner>Chuẩn bị câu {currentQuestion.display_order}! Sẵn sàng...</Banner>}
            ...
            <>
              ...
              {isReveal && myAnswer && (
                <div className={`p-3 rounded-lg ${myAnswer.is_correct ? "bg-emerald-100 text-emerald-900" : "bg-rose-100 text-rose-900"}`}>
                  {myAnswer.is_correct
                    ? `✓ Đúng! Bạn được ${myAnswer.points_awarded} điểm.`
                    : `✗ Sai. Đáp án đúng là ${currentQuestion.correct_option}.`}
                </div>
              )}
            </>
          )}
